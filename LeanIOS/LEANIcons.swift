...
@objc class LEANIcons: NSObject {
    ...
    @objc public class func imageForIconIdentifier(_ name: String, size: CGFloat) -> UIImage? {
        return sharedIcons.imageFor(iconName: name, size: size)
    }
    ...
    private func imageFor(iconName: String, size: CGFloat) -> UIImage? {
        // separating icon code (fab fa-apple --> apple)
